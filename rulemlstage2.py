...
# ---------- HELPERS: TradingView clickable link ----------
def tradingview_link(ticker):
    tv_ticker = ticker.replace('.NS', '')
    url = f"https://www.tradingview.com/chart/?symbol=NSE:{tv_ticker}"
    return f"[{ticker}]({url})"

def display_with_tradingview(df, key="Ticker", nrows=20):
    if df.empty:
        st.info("No signals.")
        return
    df = df.copy()
    df['TradingView'] = df[key].apply(tradingview_link)
    # Put 'TradingView' as first column, remove original 'Ticker' to avoid duplication
    cols = ['TradingView'] + [col for col in df.columns if col not in ('TradingView', key)]
